...
public enum AudioTools {
    ...
    public static func createLoopedAudio(input: URL, output: URL, minimumDuration: TimeInterval) async throws -> URL {
        ...
        guard duration * 2 < minimumDuration else {
            throw NSError(description: "input duration is too long (\(duration)) sec and doesn't need to loop. VS minimumDuration \(minimumDuration) sec")
        }
        ...
        let numberOfDuplicates: Int = Int(minimumDuration / duration) + 1
